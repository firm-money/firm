...
contract DeployLiquity2Script is DeployGovernance, UniPriceConverter, StdCheats, MetadataDeployment, Logging {
    ...
    function run() external {
        ...
        uint256 epochStart = vm.envOr(
            "EPOCH_START",
            (block.chainid == 373 ? _latestUTCMidnightBetweenWednesdayAndThursday() : block.timestamp) - EPOCH_DURATION
        );
        ...
        if (block.chainid == 1660990954) {
            ...
            for (uint256 i = 0; i < deployed.contractsArray.length; ++i) {
                if (i == 0) { // WETH
                    ...
                    PriceFeedTestnet(address(deployed.contractsArray[i].priceFeed)).setPrice(0.01 ether);
                } else if (i == 4) { // LINEA
                    PriceFeedTestnet(address(deployed.contractsArray[i].priceFeed)).setPrice(0.003 ether);
                } else { // sGUSD
                    assert(i == 5);
                    PriceFeedTestnet(address(deployed.contractsArray[i].priceFeed)).setPrice(1 ether);
                }
                // PriceFeedTestnet(address(deployed.contractsArray[i].priceFeed)).setPrice(2_000 ether);
    ...
    function _deployAndConnectContracts(
        ...
        // Collaterals
        if (block.chainid == 373 && !useTestnetPriceFeeds) {
            // mainnet
            // ETH
